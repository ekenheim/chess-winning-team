...
Useful views: `git log --oneline --all` (both users), `git log --oneline --grep='^\[keep\]\|^\[FULL\]'` (the ascent), `git tag -l 'beat-*'` (the proofs), and `evidence/README.md` on main (every claimed Elo with the games behind it, re-verified).
...
7. Record the results: `git add games analysis` and rewrite the commit message with the real result (`git commit --amend`) — the commit is the log entry. Push it after step 8/9 (`git push --force-with-lease`, needed because of the amend) so the other machine sees it. Just before pushing, **update the Elo chart and scoreboard** in the same commit: `git fetch origin && python tools/progress.py && git add progress.svg README.md && git commit --amend --no-edit` (it reads the final result line from the log, so run it only after the message is written)
   After pushing a `[keep]`, `[FULL]` or `[ladder]` commit, **refresh the evidence on main** so every claimed Elo stays backed by its games. In the checkout of `main` (the original repo directory, not your worktree): `git pull --ff-only && git fetch origin && python tools/evidence.py && git add evidence && git commit -m "Evidence: <tag> <sha>" && git push`. If the push is rejected because the other machine refreshed first, run `git pull --rebase -X theirs`, re-run the script, `git add evidence && git commit --amend --no-edit` and push again. The script rebuilds `evidence/` from the log, so running it again is always safe. If it reports an UNVERIFIED claim (games missing, the Elo doesn't recompute, a move over the budget, or a proof that isn't a 5 s win), fix the record before the next experiment.
...
9. If not, undo the engine change but keep the record: the experiment commit carries the `[discard]` (or `[crash]`) result line, then revert **only the engine**: `git revert --no-commit HEAD && git checkout HEAD -- games analysis progress.svg README.md && git commit --no-edit`. A plain `git revert` would also delete the run's games and analysis from the branch, and every game must stay on the branch so it can be replayed. Engine code is back where it started; the attempt, its games and its analysis stay in the tree
